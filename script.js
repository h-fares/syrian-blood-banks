(async () => {
    ...
    const bloodBanks = [
        ...
        { name: "ريف دمشق", lat: 33.5164, lon: 36.5936, capacity: "مركز إسعاف الهلال الأحمر العربي السوري في دوماز المجمع الطبي للهلال الأحمري العربي السوري في جرمانا" },
    ...
    Highcharts.mapChart('container', {
        chart: {
            map: topology,
            backgroundColor: "#1B1F2A" // Deep Blue Gray Background
        },

        title: {
            text: 'بنوك الدم في المحافظات السورية',
            style: {
                color: "#00C8FF", // Cyan Title
                fontSize: "22px"
            }
        },
        ...
        tooltip: {
            ...
            pointFormat: `<b >{point.name}</b><br>
                <span style="font-size:18px; color:#2FFFAD">{point.capacity}</span>`,
            positioner: function () {
                return { x: 10, y: 250 };
            }
        },
        ...
        series: [{
            name: 'Syria',
            mapData: topology,
            color: "#324A5F",  // Soft Teal Gray Land
            borderColor: "#6B798F",  // Light Steel Blue Borders
            nullColor: "#2A2F3A", // Darker Background
            states: {
                hover: { color: "#00C8FF" }  // Electric Cyan Hover
            }
        }, {
            type: 'mappoint',
            name: 'Blood Banks',
            color: "#2FFFAD", // Bright Neon Green for markers
            dataLabels: {
                enabled: true,
                format: '{point.name}',
                color: '#DDE2E8', // Light gray text
                style: {
                    fontSize: '14px',
                    textOutline: 'none'
                }
